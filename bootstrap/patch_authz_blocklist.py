...
def _is_user_authorized_file() -> pathlib.Path:
    """Locate the file holding `_is_user_authorized`.

    It lived in gateway/run.py until the upstream "god-file decomposition" lifted
    it into gateway/authz_mixin.py. Support BOTH so this patch is robust across
    Hermes base versions: prefer authz_mixin.py when it exists AND carries the
    anchor, else fall back to run.py.
    """
    if AUTHZ.exists() and AUTHZ_OLD in AUTHZ.read_text():
        return AUTHZ
    return RUN_PY
...
def main() -> None:
    ...
    authz_file = _is_user_authorized_file()
    apply(authz_file, AUTHZ_OLD, AUTHZ_NEW, f"_is_user_authorized ({authz_file.name}): block beats allow")
