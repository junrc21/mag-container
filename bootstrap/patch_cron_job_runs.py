...
NEW_START_PROCESS = (
    ...
    "            # MAG: admin block guard — rotina de tenant bloqueado nao roda.\n"
    "            # Ver a nota completa no gemeo abaixo, em run_one_job.\n"
    "            if _mag_check_tenant_access().status is _MagAccessStatus.BLOCKED:\n"
    "                return True\n"
...
NEW_START_RUN_ONE = (
    ...
    "    # MAG: admin block guard — rotina de tenant bloqueado nao roda.\n"
    "    #\n"
    "    # NAO pausa o job, de proposito. Pausar e persistente e exigiria o cliente\n"
    "    # reativar em Rotinas — e ele nem consegue entrar no painel enquanto estiver\n"
    "    # bloqueado. Pular faz o desbloqueio devolver as rotinas sozinho, no proximo\n"
    "    # tick, sem ninguem precisar lembrar de nada.\n"
    "    #\n"
    "    # E nao notifica o canal: avisar entregaria justamente a informacao que o\n"
    "    # bloqueio existe para reter.\n"
    "    if _mag_check_tenant_access().status is _MagAccessStatus.BLOCKED:\n"
    "        return True\n"
    "    # MAG: authoritative cron credit guard — applies to agent and no-agent jobs.\n"
    "    _mag_credit_check = _mag_check_authoritative_credits()\n"
