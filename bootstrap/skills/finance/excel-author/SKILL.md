---
name: excel-author
description: Build auditable Excel workbooks headless with openpyxl — blue/black/green cell conventions, formulas over hardcodes, named ranges, balance checks, sensitivity tables. Use for financial models, audit outputs, reconciliations, and any other spreadsheet the user asks for.
version: 1.0.0
author: Anthropic (adapted by Nous Research, MAG)
license: Apache-2.0
platforms: [linux]
metadata:
  hermes:
...
Produce an .xlsx file on disk using `openpyxl` (already installed — no setup step needed). Follow the banker-grade conventions below so the model is auditable, flexible, and reviewable by someone other than the person who built it. For a simple spreadsheet (a list, a small table) most of this is overkill — use only what fits the request, skip the modeling ceremony for anything that isn't actually a financial model.

**Just `import openpyxl` and use it — do not hand-roll the .xlsx ZIP/XML structure yourself.** `openpyxl` is verified installed in this environment; there is no need to write `Content_Types.xml`/`workbook.xml`/etc. by hand or to second-guess whether the import will work. Hand-rolling OOXML is slower (more code, more reasoning, more chances to get the schema subtly wrong) and gives up every convention below for free — `openpyxl` gets you real fonts, comments, named ranges, and formulas with a few lines. If `import openpyxl` genuinely fails, say so and stop — don't silently fall back to writing the ZIP by hand.

Adapted from Anthropic's `xlsx-author` and `audit-xls` skills in the [anthropics/financial-services](https://github.com/anthropics/financial-services) repo. The MCP / Office-JS / Cowork-specific branches of the originals are dropped — this skill assumes headless Python, and delivery goes through the `MEDIA:` convention (see bottom).

## Output contract

- Write to `/opt/data/workspace/<name>.xlsx`. Create `/opt/data/workspace/` if it does not exist.
- One logical model per file. Do not append to an existing workbook unless explicitly asked.
- See "Sending the file to the user" below for how to deliver it — do not just describe the path in prose.

## Core conventions (non-negotiable for financial models)
...
wb.save("/opt/data/workspace/model.xlsx")
```
...
## Formulas recalculate automatically — nothing to do here

openpyxl writes formula strings but does not compute them itself — that's normal, not a bug. Excel, Numbers, and Google Sheets all recalculate every formula the moment the user opens the file, so there is nothing extra to run before delivery. (There is no LibreOffice in this environment for a headless pre-recalc step — don't reach for one, it isn't needed for a file the user opens themselves.)
...
- Users in a live Excel session with an Office MCP available — drive their live workbook instead (not applicable in this environment — there is no such MCP here).
- Pure tabular data export with no formulas — plain rows in a couple of sheets is fine, skip the blue/black/green ceremony.
...
```
MEDIA:/opt/data/workspace/model.xlsx
```

Example full response:
```
Pronto! Aqui está a planilha gerada.
MEDIA:/opt/data/workspace/model.xlsx
```
...
Output path and delivery mechanism (`MEDIA:` tag) adapted for MAG's headless agent runtime.
